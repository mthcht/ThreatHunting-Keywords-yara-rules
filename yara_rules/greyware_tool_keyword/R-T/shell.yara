rule shell
{
    ...
    strings:
        ...
        // Description: Reverse Shell Command Line
        // Reference: https://github.com/SigmaHQ/sigma/blob/master/rules/linux/lnx_shell_susp_rev_shells.yml
        $string3 = /rm\s\-f\sbackpipe.*\smknod\s\/tmp\/backpipe\sp\s\&\&\snc\s/ nocase ascii wide
        ...
        // Description: Reverse Shell Command Line
        // Reference: https://github.com/SigmaHQ/sigma/blob/master/rules/linux/lnx_shell_susp_rev_shells.yml
        $string5 = /socket\(S.*PF_INET.*SOCK_STREAM.*getprotobyname\(.*tcp.*\)\).*if\(connect\(S.*sockaddr_in\(\$p.*inet_aton\(\$i\)\)\)\)/ nocase ascii wide
        // Description: Reverse Shell Command Line
        // Reference: https://github.com/SigmaHQ/sigma/blob/master/rules/linux/lnx_shell_susp_rev_shells.yml
        $string6 = /STDIN\-\>fdopen\(\$c.*r\).*\$~\-\>fdopen\(\$c.*w\).*system\$_\swhile\<\>/ nocase ascii wide
        // Description: Reverse Shell Command Line
        // Reference: https://github.com/SigmaHQ/sigma/blob/master/rules/linux/lnx_shell_susp_rev_shells.yml
        $string7 = /uname\s\-a.*\sw.*\sid.*\s\/bin\/bash\s\-i/ nocase ascii wide
