rule powershell
{
    ...
    strings:
        ...
        // Reference: N/A
        $string16 = /Get\-ADComputer\s.*\s\-Properties\sPrincipalsAllowedToDelegateToAccount/ nocase ascii wide
        ...
        $string17 = /http:\/\/.*Microsoft\.ActiveDirectory\.Management\.dll/ nocase ascii wide
        // Description: redteam technique - import the ActiveDirectory module without the need to install it on the current computer - the dll has been extracted from a Windows 10 x64 with RSAT installed
        // Reference: https://github.com/mthcht/Purpleteam/blob/main/Simulation/Windows/ActiveDirectory/Bruteforce.ps1
        $string18 = /https:\/\/.*Microsoft\.ActiveDirectory\.Management\.dll/ nocase ascii wide
        // Description: redteam technique - import the ActiveDirectory module without the need to install it on the current computer - the dll has been extracted from a Windows 10 x64 with RSAT installed
        // Reference: https://github.com/mthcht/Purpleteam/blob/main/Simulation/Windows/ActiveDirectory/Bruteforce.ps1
        $string19 = /Import\-Module\s.*Microsoft\.ActiveDirectory\.Management\.dll/ nocase ascii wide
        ...
        $string23 = /Remove\-ItemProperty\s.*HKLM:\\SOFTWARE\\YourSoftware\\Schedule\\TaskCache\\Tree\\.*\s\-Name\s.*SD/ nocase ascii wide
        ...
        $string24 = /powershell\.exe\s\-nop\s\-c\s\"start\-job\s.*Import\-Module\sBitsTransfer.*\$env:temp.*GetRandomFileName\(\).*Start\-BitsTransfer\s\-Source\s\'http.*Remove\-Item.*Receive\-Job/ nocase ascii wide
