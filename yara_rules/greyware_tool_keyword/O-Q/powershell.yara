rule powershell
{
    ...
    strings:
        ...
        // Reference: N/A
        $string1 = /\\powershell\.exe.*\s\+\=\shidden/ nocase ascii wide
        // Description: command aiming to hide a file. It can be performed with  powershell on a WINDOWS machine with command option =hidden
        // Reference: N/A
        $string2 = /\\powershell\.exe.*\s\+\=hidden/ nocase ascii wide
        // Description: command aiming to hide a file. It can be performed with  powershell on a WINDOWS machine with command option =hidden
        // Reference: N/A
        $string3 = /\\powershell\.exe.*\s\=\shidden/ nocase ascii wide
        // Description: command aiming to hide a file.  It can be performed with  powershell on a WINDOWS machine with command option =hidden
        // Reference: N/A
        $string4 = /\\powershell\.exe.*\s\=hidden/ nocase ascii wide
        // Description: Exclude powershell from defender detections
        // Reference: N/A
        $string5 = /Add\-MpPreference\s\-ExclusionProcess\s.*\\Windows\\System32\\WindowsPowerShell\\v1\.0\\powershell\.exe/ nocase ascii wide
        // Description: allows all users to access all computers with a specified configuration
        // Reference: N/A
        $string6 = /Add\-PswaAuthorizationRule\s\-UsernName\s\\.*\s\-ComputerName\s\\.*\s\-ConfigurationName\s\\/ nocase ascii wide
        ...
        $string7 = /Clear\-RecycleBin\s\-Force\s\-ErrorAction\sSilentlyContinue/ nocase ascii wide
        ...
        $string8 = /Find\-LocalAdminAccess\s\-Verbose/ nocase ascii wide
        ...
        $string9 = /gci\s\-h\sC:\\pagefile\.sys/ nocase ascii wide
        // Description: AppLocker Get AppLocker policy
        // Reference: https://hideandsec.sh/books/cheatsheets-82c/page/active-directory
        $string10 = /Get\-AppLockerPolicy\s\-Effective\s/ nocase ascii wide
        // Description: Powerview Enumerate users
        // Reference: https://hideandsec.sh/books/cheatsheets-82c/page/active-directory
        $string11 = /Get\-DomainUser\s\-KerberosPreuthNotRequired\s\-Verbose/ nocase ascii wide
        // Description: PowerView get Locally logged users on a machine
        // Reference: https://hideandsec.sh/books/cheatsheets-82c/page/active-directory
        $string12 = /Get\-LoggedonLocal\s\-ComputerName\s/ nocase ascii wide
        ...
        $string13 = /Get\-MpComputerStatus/ nocase ascii wide
        ...
        // Reference: N/A
        $string14 = /Get\-NetGroupMember\s\-GroupName\s.*DNSAdmins/ nocase ascii wide
        // Description: PowerView Find users with SPN
        // Reference: https://hideandsec.sh/books/cheatsheets-82c/page/active-directory
        $string15 = /Get\-NetUser\s\-SPN/ nocase ascii wide
        // Description: Find local admins on the domain machines
        // Reference: https://hideandsec.sh/books/cheatsheets-82c/page/active-directory
        $string16 = /Invoke\-EnumerateLocalAdmin\s\-Verbose/ nocase ascii wide
        // Description: Check local admin access for the current user where the targets are found
        // Reference: https://hideandsec.sh/books/cheatsheets-82c/page/active-directory
        $string17 = /Invoke\-UserHunter\s\-CheckAccess/ nocase ascii wide
        // Description: C2 server to connect to a victim machine via reverse shell
        // Reference: https://github.com/reveng007/C2_Server
        $string18 = /Invoke\-WebRequest\sifconfig\.me\/ip.*Content\.Trim\(\)/ nocase ascii wide
        ...
        $string19 = /New\-Object\sSystem\.Net\.Sockets\.TCPClient\(.*\$stream\s\=\s\$client\.GetStream\(\).*\[byte\[\]\]\$bytes\s\=\s0\.\.65535/ nocase ascii wide
        ...
        $string20 = /powershell\s\-c\s.*\\windows\\system32\\inetsrv\\appcmd\.exe\slist\sapppool\s\/\@t:/ nocase ascii wide
        ...
        // Reference: N/A
        $string21 = /powershell\sNew\-ItemProperty\s\-Path\s.*HKLM:\\SOFTWARE\\Policies\\Microsoft\\Windows\sDefender.*\s\-Name\sDisableAntiSpyware\s\-Value\s1\s\-PropertyType\sDWORD\s\-Force/ nocase ascii wide
        ...
        $string22 = /powershell.*Uninstall\-WindowsFeature\s\-Name\sWindows\-Defender\-GUI/ nocase ascii wide
        // Description: Adversaries may attempt to execute powershell script from known accessible location
        // Reference: N/A
        $string23 = /Powershell\.exe\s\s\-windowstyle\shidden\s\-nop\s\-ExecutionPolicy\sBypass\s\s\-Commmand\s.*C:\\Users\\.*\\AppData\\Roaming\\/ nocase ascii wide
        ...
        $string24 = /powershell\.exe\s\-exec\sbypass\s\-noni\s\-nop\s\-w\s1\s\-C/ nocase ascii wide
        // Description: command pattern used by crackmapexec by default A swiss army knife for pentesting networks
        // Reference: https://github.com/byt3bl33d3r/CrackMapExec
        $string25 = /powershell\.exe\s\-noni\s\-nop\s\-w\s1\s\-enc\s/ nocase ascii wide
        // Description: list AV products with powershell
        // Reference: N/A
        $string26 = /root\/SecurityCenter2.*\s\-ClassName\sAntiVirusProduct/ nocase ascii wide
        // Description: Disable scanning all downloaded files and attachments
        // Reference: N/A
        $string27 = /Set\-MpPreference\s\-DisableIOAVProtection\s\$true/ nocase ascii wide
        // Description: Defense evasion technique In order to avoid detection at any point of the kill chain. attackers use several ways to disable anti-virus. disable Microsoft firewall and clear logs.
        // Reference: N/A
        $string28 = /Set\-MpPreference\s\-DisableRealtimeMonitoring\s\$true/ nocase ascii wide
        // Description: Disable AMSI (set to 0 to enable)
        // Reference: N/A
        $string29 = /Set\-MpPreference\s\-DisableScriptScanning\s1\s/ nocase ascii wide
        // Description: Jenkins Abuse Without admin access
        // Reference: https://hideandsec.sh/books/cheatsheets-82c/page/active-directory
        $string30 = /cmd\.exe\s\/c\sPowerShell\.exe\s\-Exec\sByPass\s\-Nol\s\-Enc\s/ nocase ascii wide
        ...
        // Reference: https://hideandsec.sh/books/cheatsheets-82c/page/active-directory
        $string32 = /Get\-ADGroup\s\-Filter\s.*Name\s\-like\s.*admin/ nocase ascii wide
        // Description: AD Module Enumerate principals with Constrained Delegation enabled
        // Reference: https://hideandsec.sh/books/cheatsheets-82c/page/active-directory
        $string33 = /Get\-ADObject\s\-Filter\s{msDS\-AllowedToDelegateTo\s.*\s\-Properties\smsDS\-AllowedToDelegateTo/ nocase ascii wide
        // Description: Enumerate shadow security principals mapped to a high priv group
        // Reference: https://hideandsec.sh/books/cheatsheets-82c/page/active-directory
        $string34 = /Get\-ADObject\s\-SearchBase\s.*CN\=Shadow\sPrincipal\sConfiguration.*CN\=Services.*\s\(Get\-ADRootDSE\)\.configurationNamingContext\)\s\|\sselect\s.*msDS\-ShadowPrincipalSid/ nocase ascii wide
        // Description: AD module Enumerate users
        // Reference: https://hideandsec.sh/books/cheatsheets-82c/page/active-directory
        $string35 = /Get\-ADUser\s\-Filter\s{DoesNotRequirePreAuth\s\-eq\s\$True}\s\-Properties\sDoesNotRequirePreAuth/ nocase ascii wide
        // Description: AD Module Enumerate computers with Unconstrained Delegation
        // Reference: https://hideandsec.sh/books/cheatsheets-82c/page/active-directory
        $string36 = /Get\-ADUser\s\-Filter\s{TrustedForDelegation\s\-eq\s\$True}/ nocase ascii wide
        // Description: AD Module Enumerate principals with Constrained Delegation enabled
        // Reference: https://hideandsec.sh/books/cheatsheets-82c/page/active-directory
        $string37 = /Get\-DomainComputer\s\-TrustedToAuth/ nocase ascii wide
        // Description: AD Module Enumerate principals with Constrained Delegation enabled
        // Reference: https://hideandsec.sh/books/cheatsheets-82c/page/active-directory
        $string38 = /Get\-DomainUser\s\-TrustedToAuth/ nocase ascii wide
        // Description: AD Module GroupPolicy - List of GPO in the domain
        // Reference: https://hideandsec.sh/books/cheatsheets-82c/page/active-directory
        $string39 = /Get\-GPO\s\-All/ nocase ascii wide
        // Description: Find groups in the current domain (PowerView)
        // Reference: https://hideandsec.sh/books/cheatsheets-82c/page/active-directory
        $string40 = /Get\-NetGroup\s\-FullData/ nocase ascii wide
        // Description: AD module Logon Script from remote IP
        // Reference: https://hideandsec.sh/books/cheatsheets-82c/page/active-directory
        $string41 = /Set\-ADObject\s\-SamAccountName\s.*\s\-PropertyName\sscriptpath\s\-PropertyValue\s.*\\.*\.exe/ nocase ascii wide
